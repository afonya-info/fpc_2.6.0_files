...
charset;
Const
leters:set of char = ['А'..'п','р'..'ё','0'..'9']; //vot takaya ascii interesnaya
spchar:set of char = ['-'];
bword:set of char = [' ','"','(','[','{','<'];
eword:set of char = [' ','.',',','?','!',':',';','"',')',']','}','>'];
...
BEGIN
    ...
    begin
      ...
      if not flagle then
        begin
          flag:=true;
          writeln;
          writeln('В строке должны быть только символы кириллицы. Повторите ввод:');
        end
      else
        flag:=false;
    end;
  ...
  while i< length(s1) do//zapis kazdogo slova v massi
    begin
      if ((s1[i-1] in bword) and (s1[i] in leters)) or ((i-1=0)and(s1[i] in leters))  then
        begin
          j:=i;
          n:=1;
          ...
          if (bools[i]) and (dliny[i]=dliny[j]) then// esli sovpali dliny slov togda sravnit ih posimvolno
            begin
              for m:= 1 to dliny[i] do// ╨┐╨╛╨╝╨╡╨╜╤П╤В╤М ╨┤╨╛ ╨┤╨╗╨╕╨╜╨╜╤Л ╤Б╤В╤А╨╛╨║╨╕ wmas i
                begin
                  if ((wmas[i][m] in ['А'..'П']) and (wmas[j][m] in ['а'..'п'])) or ((wmas[j][m] in ['А'..'П']) and (wmas[i][m] in ['а'..'п'])) then
                    kod1:=abs(ord(wmas[i][m])-ord(wmas[j][m]));//d b raven 32 esli bukvy odinakovye no v raznyh registrah

                  if ((wmas[i][m] in ['Р'..'Я']) and (wmas[j][m] in ['р'..'я'])) or ((wmas[j][m] in ['Р'..'Я']) and (wmas[i][m] in ['р'..'я'])) then
                    kod2:=abs(ord(wmas[i][m])-ord(wmas[j][m]));//d b raven 80 esli bukvy odinakovye no v raznyh registrah

                  if ((wmas[i][m] = 'Ё') and (wmas[j][m] = 'ё')) or ((wmas[j][m] = 'Ё') and (wmas[i][m] = 'ё')) then
                    kod3:=abs(ord(wmas[i][m])-ord(wmas[j][m]));//d b raven 1 esli bukvy odinakovye no v raznyh registrah

                  if (wmas[i][m] = wmas[j][m]) or (kod1 = 32) or (kod2 = 80) or (kod3 = 1) then // t e simvoly ili ravny ili ih cody otlich. na 32 chto sootvetstvyet ravenstvy bolshoy i malenkoi bukvy
...
slovo i po indexu ch nahodim samo slovo}
  ...
  if flag1 then
    ...
    writeln('в данной строке отсутствуют повторяющиеся слова'); // writeln(s1,'|||',wmas[1],wmas[2]);
  readln;
END.
