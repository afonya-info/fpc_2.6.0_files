...
s,s1,kk:string;
c,s1i,s1ip1:char;
wofla,wofla2,wofla3,wofla4,wofla5,flag,flag1,flagle: boolean;
    ...
    begin
      if (s1[i] in leters) then
        begin
          j:=i;
          n:=1;
          wofla:=(not((s1[i] in leters) and (s1[i+1] in eword)));
          wofla2:=((s1[i] in leters) and ((s1[i+1] in leters) or (s1[i+1] in spchar)));
          wofla3:=(s1[i] in leters);
          wofla4:=(s1[i+1] in leters);
          wofla5:=(s1[i+1] in spchar);
          s1i:=s1[i];
          s1ip1:=s1[i+1];
          while ((not((s1[i] in leters) and (s1[i+1] in eword))) and (((s1[i] in leters) or (s1[i] in spchar) ) and ((s1[i+1] in leters) or (s1[i+1] in spchar)))) do //yslovie konca slova
            begin
              wmas[k][n]:=s1[j];
              inc(i);
              inc(j);
