...
s,s1,kk:string;
c:char;
wofla,flag,flag1,flagle: boolean;
    ...
    begin
      if (s1[i] in leters) then
        begin
          j:=i;
          n:=1;
          while ((not((s1[i] in leters) and (s1[i+1] in eword))) and ((s1[i] in leters) and ((s1[i+1] in leters) or (s1[i+1] in spchar)))) do //yslovie konca slova
            begin
              wmas[k][n]:=s1[j];
              inc(i);
              inc(j);
